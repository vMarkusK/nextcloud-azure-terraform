...
locals {
  ...
  asp_name   = format("%s-%s-%s", var.asp_prefix, local.st_name, random_string.suffix.result)
  app_name   = format("%s-%s-%s", var.app_prefix, local.st_name, random_string.suffix.result)
}